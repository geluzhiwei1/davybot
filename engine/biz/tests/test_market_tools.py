...
from dawei_biz.tools import market_tools as mt
...
class TestM1Registration:
    def test_tools_group_catalog_consistent(self):
        ...
        group = set(get_all_tool_groups()["market"]["custom_tools"])
        assert group == names, "工具组['market'] 与 MARKET_TOOLS 不一致"
        catalog = {c.name for c in get_catalog() if c.group == "market"}
