...
from dawei_biz.tools import research_tools as rt
...
class TestM1Registration:
    def test_tools_group_catalog_consistent(self):
        ...
        group = set(get_all_tool_groups()["research"]["custom_tools"])
        assert group == names, "工具组['research'] 与 RESEARCH_TOOLS 不一致"
        catalog = {c.name for c in get_catalog() if c.group == "research"}
