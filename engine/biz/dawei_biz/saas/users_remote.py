...
from dawei_biz.saas.remote.nat_service import (
    NATService,
    NATTunnelInfo,
    get_nat_service,
)
...
# 原挂载于核心 users 路由（prefix=/api/users）之下 → /api/users/me/remote。
# 6b-2 迁库后经 S3 聚合器直接挂载，prefix 必须自带 /api/users 段以保持路径不变。
router = APIRouter(prefix="/api/users/me/remote", tags=["Remote Services"])
