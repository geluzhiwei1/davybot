...
"""Legal knowledge tools — nn-kb-searcher legal-ext API 客户端工具（biz 侧）。

阶段六 6a 自 knowledge_tool.py 拆出（拆库方案 §18.2 桶 1），6b 将随 dawei_biz 迁移。
多租户：JWT 按请求从 local_context 注入（Bearer 透明注入，永不进 LLM 上下文）。
"""
...
# S2 工具组注册源（dawei_biz/__init__ 经此合并入核心 knowledge 组，防名称漂移）
LEGAL_KNOWLEDGE_TOOLS: list[CustomBaseTool] = [
    LegalSearchTool,
    LegalAskTool,
    LegalDocumentTool,
...
LEGAL_TOOL_NAMES: set[str] = {t().name for t in LEGAL_KNOWLEDGE_TOOLS}  # module-level convenience (tests)
