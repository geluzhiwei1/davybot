...
def get_valid_modes(workspace_path: str | None = None) -> list[str]:
    ...
    P6 修复：改经 ModeRegistry 单例读取，不再 ad-hoc ModeManager（绕过 300s TTL 缓存）。
    ...
    try:
        ...
        registry = get_registry(workspace_path)

        # 内置模式在前，自定义模式在后（自定义按 priority 降序，§4.5）
        builtin = get_builtin_modes()
        custom = [m.slug for m in registry.all() if m.slug not in builtin]
...
def get_all_mode_info(workspace_path: str | None = None) -> dict[str, dict]:
    """获取所有模式的详细信息（P6：经 ModeRegistry 单例读取）
    ...
    try:
        ...
        for mode in registry.all():
            mode_info[mode.slug] = {
                ...
                "source": mode.source,
                "is_builtin": is_builtin_mode(mode.slug),
                "is_framework": mode.is_framework,
            }
