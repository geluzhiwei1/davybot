...
class CircuitBreaker:
    ...
    def _is_retryable_error(self, error_str: str) -> bool:
        ...
        重试策略（FAST FAIL 原则）：
        - 429 rate limit → 不在 circuit_breaker 层重试。rate limit 窗口（60s）远超
          指数退避周期（1~16s），重试只会浪费请求配额。直接抛出，让上层
          （task_node_executor → 前端提示用户等待 60s 后手动重试）。
        - 400/401/402/404 → 永远不可重试（请求本身有误）。
        - 500/502/503/timeout/connection → 可重试（服务端临时故障）。
        """
        retryable_patterns = [
            "timeout",  # 超时
