...
class BaseClient(LlmApi, ABC):
    ...
    def get_provider_name(self) -> str:
        ...
        默认实现从类名提取，但子类应覆盖此方法以提供稳定标识。
        例如: OpenaiCompatibleClient → override → "openai-compatible"
        """
    ...
    def _get_client_session(self) -> ClientSession:
        ...
        if self._session is None or self._session.closed:
            ...
            # 创建会话
            # TCPConnector(force_close=True)：不复用连接，每次请求新建 TCP+TLS。
            # 根因修复——aiohttp 共享 session 会复用”被取消/未读完的流”留下的脏连接，
            # 下一次请求读到残留字节 → 200 头之后 0 字节 Connection closed。
            # force_close 让连接用完即弃，行为对齐 curl（已验证 curl 用同样请求可完整流式）。
            connector = TCPConnector(enable_cleanup_closed=True, limit=100, force_close=True)
            session_kwargs = {
                "timeout": timeout,
                "headers": headers,
                "connector": connector,
