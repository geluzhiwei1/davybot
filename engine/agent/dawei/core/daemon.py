...
def get_pid_file_path(port: int, pid_dir: Path | None = None) -> Path:
    ...
    Args:
        ...
        pid_dir: Custom PID directory (defaults to ~/.normnomos/run)
...
def create_log_file(log_dir: Path | None = None, port: int = 8431) -> Path:
    """Create log directory and return log file path.

    Args:
        log_dir: Custom log directory (defaults to ~/.normnomos/logs)
