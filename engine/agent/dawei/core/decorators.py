...
class SafeOperationConfig:
    ...
    def __init__(
        ...
        include_error_in_fallback: bool = False,
    ):
...
def safe_operation(
    ...
    Args:
        ...
        include_error_in_fallback: 字符串 fallback 是否附加真实异常信息。
            默认 False（保持历史行为）；工具类装饰器应开启，让 LLM
            拿到异常原因以自纠，而不是只收到 26 字符的裸错误串。
    ...
    config = SafeOperationConfig(
        ...
        include_error_in_fallback=include_error_in_fallback,
    )
...
async def _execute_with_error_handling_async(
    ...
    if last_exception:
        ...
        if config.fallback_value is not None:
            return _augment_fallback(config, last_exception)
        raise last_exception
    return config.fallback_value


def _augment_fallback(config: SafeOperationConfig, exception: Exception) -> Any:
    """字符串 fallback 附加真实异常信息（FAST FAIL：错误不许裸奔）。

    2026-09-17 事故：smart_text_edit 兜底返回 26 字符的
    "Error: Failed to edit file"，LLM 与监控均无法定位根因
    （实为 TypeError 参数名不匹配）。开启 include_error_in_fallback
    后，LLM 可拿到异常类型与消息进行自纠。

    JSON 形态的 fallback（如命令工具的 {"status": "error", ...}）注入
    "error_detail" 字段而非文本追加 —— 保持 JSON 可解析（调用方
    json.loads 不被破坏）。
    """
    if not config.include_error_in_fallback:
        return config.fallback_value
    if not isinstance(config.fallback_value, str):
        return config.fallback_value
    detail = f"{type(exception).__name__}: {exception!s}"
    if len(detail) > 500:
        detail = detail[:500] + "..."
    if config.fallback_value.lstrip().startswith("{"):
        try:
            payload = json.loads(config.fallback_value)
        except (json.JSONDecodeError, ValueError):
            payload = None
        if isinstance(payload, dict):
            payload["error_detail"] = detail
            return json.dumps(payload, ensure_ascii=False)
    return f"{config.fallback_value} [{detail}]"
