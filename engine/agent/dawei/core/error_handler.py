...
class RecoveryExecutor:
    ...
    async def _execute_fallback(
        ...
        if isinstance(error.error, ToolExecutionError):
            # 2026-09-14: str(e) 可能为空 (如 __str__ 未实现的包装异常) →
            # 回退 repr, 绝不让前端收到 {"success": false, "error": ""} 无信息误报
            err_text = str(error.error) or repr(error.error)
            return RecoveryResult(
                success=True,
                strategy=RecoveryStrategy.FALLBACK,
                fallback_value={"success": False, "error": err_text},
...
class ErrorHandler:
    ...
    async def handle_error(
        ...
        # 1.5. Auth failures (401) must never retry — override strategy immediately.
        # 401 means credentials are invalid/expired; retrying just burns resources.
        error_msg_lower = str(error).lower()
        if (
            "401" in str(error)
            or "unauthorized" in error_msg_lower
            or "authentication failed" in error_msg_lower
        ):
            classified_error = ClassifiedError(
                error=error,
...
def handle_errors(component: str, operation: str = None):
    ...
    def decorator(func):
        ...
        def sync_wrapper(*args, **kwargs):
            """DEPRECATED: sync error handling via new_event_loop().

            This wrapper creates and destroys an asyncio event loop on every error,
            which is expensive. Async functions should use @handle_errors via the
            async_wrapper path instead. This exists only for legacy sync callers.
            """
            try:
                return func(*args, **kwargs)
            except Exception as e:
                # Check if in async context — if so, we cannot create a new loop;
                # propagate to the running loop's exception handler instead.
                try:
