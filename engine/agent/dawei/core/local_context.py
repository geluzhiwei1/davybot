...
# 当前用户的 JWT（多租户：每个请求/会话独立）。
# 由 chat handler 从 user_message.metadata["auth_token"] 写入，
# 供原生业务工具（kb-searcher / sanctions）以 Bearer 注入到内部服务调用。
# 永不作为工具参数暴露给 LLM。
...
def set_auth_token(auth_token: str | None) -> None:
    """设置当前用户 JWT（多租户业务工具用）。"""
