...
class MemoryGardener:
    ...
    def __init__(
        ...
        Args:
            ...
            consolidation_enabled: 是否启用 LLM consolidation（默认 False，仅 decay+archive，
                零 LLM 成本；显式开启后每小时跑一次 consolidation）
    ...
    async def _gardener_loop(self):
        ...
        while self._running:
            try:
                ...
                # Consolidate recent memories（需 llm_provider 且显式启用，控 LLM 成本）
                if self.llm_provider and self.consolidation_enabled:
                    await self._consolidate_memories()

                # Archive expired memories
                await self._archive_expired()

                # Refresh rolling summary (LLM required for generation)
...
AVOIDS: [things to avoid based on the data]"""
            ...
            response = await self.llm_provider.process_message(
    ...
    async def _refresh_summary(self):
        """Refresh the rolling memory summary after decay + archival.

        Uses MemoryConsolidator.generate_summary() to produce a concise
        overview of high-energy memories, then persists it via
        MemoryGraph.save_summary().
        """
        try:
            from dawei.memory.consolidator import MemoryConsolidator

            consolidator = MemoryConsolidator(self.memory_graph, self.llm_provider)
            summary = await consolidator.generate_summary()
            if summary:
                await self.memory_graph.save_summary(summary)
                self.logger.debug("Memory summary refreshed by gardener")
        except Exception as e:
            self.logger.debug(f"Summary refresh skipped: {e}")
