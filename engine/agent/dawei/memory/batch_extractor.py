...
class BatchMemoryExtractor:
    ...
    提取的记忆自动分级存储：
    - 用户偏好 (PREFERENCE) 和策略 (STRATEGY) → user_memory_graph（跨工作区共享）
    - 工作上下文 (CONTEXT)、流程 (PROCEDURE)、事实 (FACT) → workspace memory_graph

    Usage:
        extractor = BatchMemoryExtractor(
            workspace_path="/path/to/workspace",
            memory_graph=memory_graph,
            user_memory_graph=user_memory_graph,  # 可选
        )
    ...
    def __init__(
        ...
        llm_service=None,
        user_memory_graph: MemoryGraph | None = None,
        ...
        Args:
            ...
            memory_graph: 工作区级 MemoryGraph 实例
            llm_service: LLM服务（可选，用于智能提取）
            user_memory_graph: 用户级 MemoryGraph 实例（可选，跨工作区共享）
            user_id: 用户 ID（用于 auto-memory 用户级路径）
        """
        self.workspace_path = Path(workspace_path).resolve()
        self.memory_graph = memory_graph
        self.user_memory_graph = user_memory_graph
        ...
        try:
            ...
            for conv_file in conversation_files:
                try:
                    ...
                    if memories:
                        # 按级别分发到对应 MemoryGraph (via consolidator for dedup)
                        from dawei.memory.consolidator import MemoryConsolidator

                        ws_candidates = []
                        user_candidates = []
                        for memory in memories:
                            if self._should_go_to_user_level(memory) and self.user_memory_graph:
                                user_candidates.append(memory)
                            else:
                                ws_candidates.append(memory)

                        # 写入 auto-memory markdown 文件 (并行写入, 不影响 MemoryGraph)
                        try:
                            self._write_to_auto_memory(memories)
                        except Exception as ame:
                            self.logger.warning(f"Auto-memory write failed (non-fatal): {ame}")

                        ws_count = 0
                        user_count = 0

                        if ws_candidates:
                            cons = MemoryConsolidator(self.memory_graph, self.llm_service)
                            r = await cons.consolidate(ws_candidates)
                            ws_count = r["added"] + r["updated"]

                        if user_candidates:
                            cons = MemoryConsolidator(self.user_memory_graph, self.llm_service)
                            r = await cons.consolidate(user_candidates)
                            user_count = r["added"] + r["updated"]
                        ...
                        if user_count > 0:
                            self.logger.debug(
                                f"  {conv_file.name}: {ws_count} workspace + {user_count} user-level"
                            )
...
提取事实："""
    ...
    def _should_go_to_user_level(self, memory: MemoryEntry) -> bool:
        """判断记忆是否应存入用户级（跨工作区共享）MemoryGraph。

        分类规则：
        - PREFERENCE（偏好）: 用户偏好是跨工作区的（"我喜欢 Python"）
        - STRATEGY（策略）: 学到的策略可复用（"使用 FastAPI 做 API"）
        - FACT 且主体为 "User" 且谓词为通用习惯（uses/needs）: 跨工作区
        - 其余（CONTEXT、PROCEDURE、项目相关 FACT）: 工作区级
        """
        if memory.memory_type in (MemoryType.PREFERENCE, MemoryType.STRATEGY):
            return True

        if (
            memory.memory_type == MemoryType.FACT
            and memory.subject.lower() == "user"
            and memory.predicate.lower() in ("uses", "needs", "requires", "has")
        ):
            return True
    ...
    def _memory_type_to_auto_category(self, memory: MemoryEntry) -> str:
        ...
        mapping = {
            ...
            MemoryType.STRATEGY: "procedures",
            MemoryType.PROCEDURE: "procedures",
            MemoryType.FACT: "facts",
            MemoryType.CONTEXT: "facts",
            MemoryType.EPISODE: "debugging",
        }
        return mapping.get(memory.memory_type, "facts")

    def _write_to_auto_memory(self, memories: List[MemoryEntry]) -> int:
        """将提取的记忆写入 auto-memory markdown 文件.
        ...
        ws_dir = workspace_auto_memory_dir(self.workspace_path)
        count = 0
        ...
        for mem in memories:
            try:
                ...
                self.logger.warning(f"Failed to write auto-memory entry: {e}")

        if count:
            self.logger.info(f"Wrote {count} entries to auto-memory")
        return count
