...
class UserUIContext:
    ...
    # 用户在 UI 明确选择的模型来源：official=官方网关 / local=本地自配 provider。
    # 后端据此明确路由，不靠 model id 猜测。
    current_llm_source: str | None = None
    # 仅 local：对应的 provider 配置名（_configs 的 key），用于精确命中本地 provider。
    current_config_name: str | None = None
    conversation_id: str | None = None
    current_experts: List[str] = field(default_factory=list)  # 当前工作区所选专家/Agent 名称列表
    action: str | None = None  # 用户前端操作意图，如 "summarize"、"analyze" 等
    current_user_language: str | None = None  # 用户在 UI 选择的语言偏好（如 "zh-CN"、"en-US"），用于指导 LLM 回复语言
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_llm_source": self.current_llm_source,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "UserUIContext":
        ...
        return cls(
            ...
            current_llm_source=data.get("current_llm_source"),
            current_config_name=data.get("current_config_name"),
            conversation_id=data.get("conversation_id"),
            current_experts=data.get("current_experts", []),
            action=data.get("action"),
            current_user_language=data.get("current_user_language"),
        )
