...
class BaseLLMMessage(BaseModel, ABC):
    ...
    def to_openai_format(self) -> Dict[str, Any]:
        ...
        # 【关键修复】role="tool" 的消息必须始终包含 tool_call_id 字段，
        # 即使值为空。OpenAI/DeepSeek API 要求 tool 消息必须有此字段，
        # 缺失会导致 400 错误: "missing field `tool_call_id`"
...
class ToolMessage(BaseLLMMessage):
    ...
    # X1 fix: governance metadata (optional, only present when result was truncated)
    # Not sent to LLM — used for session persistence and audit trail
    governance_meta: Dict[str, Any] | None = Field(None, description="治理元数据（截断信息）")

    def to_dict(self) -> Dict[str, Any]:
        """转换为字典格式"""
        d = self.to_openai_format()
        if self.governance_meta:
            d["governance_meta"] = self.governance_meta
        return d
    ...
    def from_openai_format(cls, data: Dict[str, Any], message_id: str | None = None) -> "ToolMessage":
        ...
        return cls(
            ...
            name=data.get("name"),
            timestamp=timestamp,
            governance_meta=data.get("governance_meta"),  # X1: restore governance meta
        )
