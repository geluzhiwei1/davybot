...
class ScheduledTask:
    ...
    # 终止条件
    end_time: datetime | None = None  # 到此时间后不再执行 (所有类型通用)

    def to_dict(self, include_workspace_info: bool = False) -> Dict[str, Any]:
        """序列化为字典

        Args:
            include_workspace_info: 是否包含工作空间名称等附加信息
        """
        d = asdict(self)
        ...
        # 从 metadata 中提取 last_execution_conversation_id
        if self.metadata:
            d["last_execution_conversation_id"] = self.metadata.get("last_execution_conversation_id")
        else:
            d["last_execution_conversation_id"] = None

        return d

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "ScheduledTask":
        """从字典反序列化（自动忽略不属于 dataclass 的额外字段）"""
        data = data.copy()
        ...
        # 仅保留 dataclass 已知字段，忽略额外字段（如 workspace_name 等）
        import dataclasses
    ...
    def should_repeat(self) -> bool:
        ...
        # 检查 end_time 终止条件 (所有类型通用)
        if self.end_time and now >= self.end_time:
            return False

        # Cron 任务: 检查 max_repeats (对 CRON 也生效)
