...
async def get_workspace_config(workspace_id: str, request: Request) -> WorkspaceConfigResponse:
    """获取工作区配置"""
...
@router.get("/{workspace_id}/skills-tools/effective")
async def get_skills_tools_effective(workspace: UserWorkspace = Depends(get_user_workspace)):
    """override-or-inherit 合并 user 默认 + workspace 覆盖的 Skills/Tools 开关。

    字段级 merge：workspace 非 None 字段覆盖 user 默认（同 effective_policy.merge_override 语义）。
    供前端 Skills/Tools tab 三态展示（继承默认 / 覆盖）。
    """
    if not workspace.is_initialized():
        await workspace.initialize()
    ...
    user_cfg = load_user_skills_tools(workspace.user_id)
    user_skills = user_cfg.get("skills", {}) or {}
    user_tools = user_cfg.get("tools", {}) or {}

    ws_config = await load_workspace_config(workspace)
    ws_full = ws_config.model_dump_custom()
    ws_skills = ws_full.get("skills", {}) or {}
    ws_tools = ws_full.get("tools", {}) or {}

    # 字段级 override-or-inherit：workspace 非 None 字段覆盖 user 默认
    eff_skills = {**user_skills, **{k: v for k, v in ws_skills.items() if v is not None}}
    eff_tools = {**user_tools, **{k: v for k, v in ws_tools.items() if v is not None}}

    return {
        "success": True,
        "default": {"skills": user_skills, "tools": user_tools},
        "override": {"skills": ws_skills, "tools": ws_tools},
        "effective": {"skills": eff_skills, "tools": eff_tools},
    }


@router.get("/{workspace_id}/memory/effective")
async def get_memory_effective(workspace: UserWorkspace = Depends(get_user_workspace)):
    """override-or-inherit：合并 user memory 默认 + workspace memory 覆盖。"""
    if not workspace.is_initialized():
        await workspace.initialize()
    from dawei.api.users.memory import load_user_memory

    user = load_user_memory(workspace.user_id)
    ws_config = await load_workspace_config(workspace)
    ws = ws_config.model_dump_custom().get("memory", {}) or {}
    effective = {**user, **{k: v for k, v in ws.items() if v is not None}}
    return {"success": True, "default": user, "override": ws, "effective": effective}


@router.get("/{workspace_id}/knowledge/effective")
async def get_knowledge_effective(workspace: UserWorkspace = Depends(get_user_workspace)):
    """override-or-inherit：合并 user knowledge 默认 + workspace knowledge 覆盖。

    注：引擎参数运行时由每个 KB 实体的 kb.settings 驱动；workspace 级真正生效的主要是 enabled。
    """
    if not workspace.is_initialized():
        await workspace.initialize()
    from dawei.api.users.knowledge import load_user_knowledge

    user = load_user_knowledge(workspace.user_id)
    ws_config = await load_workspace_config(workspace)
    ws = ws_config.model_dump_custom().get("knowledge", {}) or {}
    effective = {**user, **{k: v for k, v in ws.items() if v is not None}}
    return {"success": True, "default": user, "override": ws, "effective": effective}


@router.post("/{workspace_id}/config/reset")
async def reset_workspace_config(workspace_id: str, request: Request) -> WorkspaceConfigResponse:
    """重置工作区配置为默认值"""
    workspace = await get_user_workspace(workspace_id, request)
