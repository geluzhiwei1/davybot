...
async def reload_workspace_config(
    ...
    # 重新加载 Skills
    if payload.config_type in ["all", "skills"]:
        total_count += 1
        if workspace.tool_manager and workspace.tool_manager.skill_manager:
            # 强制重新发现 skills
            workspace.tool_manager.skill_manager.discover_skills(force=payload.force)
    ...
    # 重新加载 Modes
    if payload.config_type in ["all", "modes"]:
        total_count += 1
    ...
    # 重新加载 Tools
    if payload.config_type in ["all", "tools"]:
        total_count += 1
