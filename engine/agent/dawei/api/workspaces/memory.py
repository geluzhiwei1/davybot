...
@router.post("/{workspace_id}/memory/import")
async def import_memories(
    workspace_id: str,
    file: UploadFile = File(...),
    format: str = Query("json", pattern="^(json|csv|ndjson)$"),
):
    """Import memories from uploaded file (JSON / CSV / NDJSON)"""
    db_path = _get_memory_db_path(workspace_id)
    graph = MemoryGraph(db_path)
    ...
    try:
        if format == "json":
            ...
            if not isinstance(items, list):
                errors.append("JSON must be an array of memory objects")
            else:
                for item in items:
                    try:
                        mem = MemoryEntry(
                            ...
                            object=item["object"],
                            valid_start=datetime.fromisoformat(item.get("valid_start", datetime.now(UTC).isoformat())),
                            memory_type=MemoryType(item.get("memory_type", "fact")),
                            confidence=item.get("confidence", 0.8),
                            energy=item.get("energy", 1.0),
                            keywords=item.get("keywords", []),
                            metadata=item.get("metadata", {"source": "import"}),
                        )
                        await graph.add_memory(mem)
                        imported += 1
            ...
            for row in reader:
                try:
                    mem = MemoryEntry(
                        ...
                        object=row["object"],
                        valid_start=datetime.fromisoformat(row.get("valid_start", datetime.now(UTC).isoformat())),
                        memory_type=MemoryType(row.get("type", "fact")),
    ...
    logger.info(f"Imported {imported} memories to workspace {workspace_id} (errors: {len(errors)})")
...
@router.get("/{workspace_id}/memory/md")
async def get_workspace_memory_md(workspace_id: str):
    """获取工作区级 memory.md 内容"""
    from dawei.memory.memory_file import read_memory_md, workspace_memory_md_path

    workspace_path = _get_workspace_path(workspace_id)
    if not workspace_path:
        raise HTTPException(status_code=404, detail="Workspace not found")
    path = workspace_memory_md_path(workspace_path)
    content = read_memory_md(path)
    return {"content": content, "path": str(path)}


@router.put("/{workspace_id}/memory/md")
async def update_workspace_memory_md(workspace_id: str, request: MemoryMdRequest):
    """更新工作区级 memory.md 内容"""
    from dawei.memory.memory_file import workspace_memory_md_path, write_memory_md
    ...
    logger.info("Workspace memory.md updated (workspace=%s)", workspace_id)
    return {"content": content, "path": str(path)}


# ============================================================================
# Auto Memory — Agent 自动从对话中提取的记忆 (工作区级)
#   NOTE: These specific routes MUST be declared before the generic
#   /{workspace_id}/memory/{memory_id} routes, otherwise FastAPI will
#   match "auto" as a memory_id parameter.
# ============================================================================
...
async def batch_extract_memories(
    ...
    try:
        ...
        # 初始化用户级 MemoryGraph（跨工作区共享偏好/策略）
        user_graph = None
        try:
            from dawei.api.users.memory import _user_memory_db_path

            user_db = _user_memory_db_path("default_user")
            user_graph = MemoryGraph(user_db)
        except Exception:
            pass
