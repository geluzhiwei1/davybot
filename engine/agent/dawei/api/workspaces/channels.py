...
def _ws_root(workspace_id: str) -> str:
    """workspace_id → 工作区路径（用于 per-workspace 隔离存储）。"""
    info = workspace_manager.get_workspace_by_id(workspace_id)
...
def _channel_config_file(workspace_root: str | None = None) -> Path:
    """Return channel config path.

    - ``workspace_root=None``（默认）：全局 ``{DAWEI_HOME}/configs/channels.json``（用户级 seed 源）。
    - 给定：per-workspace ``{ws}/.dawei/configs/channels.json``（真隔离）。
    """
    if workspace_root:
        return Path(workspace_root) / ".dawei" / "configs" / "channels.json"
    return get_dawei_home() / "configs" / "channels.json"


def _load_channel_configs(workspace_root: str | None = None) -> dict[str, dict[str, Any]]:
    """Load persisted channel configs from disk. per-workspace 时从全局懒 seed。"""
    path = _channel_config_file(workspace_root)
    if workspace_root and not path.exists():
        gpath = _channel_config_file(None)
        if gpath.exists():
            try:
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_bytes(gpath.read_bytes())
                logger.debug("Seeded channels config %s from global", path)
            except Exception as exc:
                logger.warning("Failed to seed channels config from global: %s", exc)
    if not path.exists():
        return {}
    try:
...
def _save_channel_configs(configs: dict[str, dict[str, Any]], workspace_root: str | None = None) -> None:
    """Persist channel configs to disk (per-workspace if workspace_root given)."""
    path = _channel_config_file(workspace_root)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def _get_saved_config(channel_type: str, workspace_root: str | None = None) -> dict[str, Any]:
    """Get saved config for a specific channel type (excludes _enabled key)."""
    configs = _load_channel_configs(workspace_root)
    saved = configs.get(channel_type, {})
    return {k: v for k, v in saved.items() if k != _ENABLED_KEY}


def _is_channel_enabled(channel_type: str, workspace_root: str | None = None) -> bool:
...
def _set_channel_enabled(channel_type: str, enabled: bool, workspace_root: str | None = None) -> None:
    ...
    _save_channel_configs(configs, workspace_root)


def _update_saved_config(
    channel_type: str,
    config: dict[str, Any],
    ...
    Args:
        ...
        workspace_root: per-workspace 存储路径
    """
    configs = _load_channel_configs(workspace_root)
...
def _build_channel_info(ct: str, workspace_root: str | None = None) -> ChannelInfo:
    """Build a ChannelInfo object for a registered channel type (per-workspace)."""
...
async def list_channels(workspace_id: str):
    ...
    _ensure_channels_registered()
    ws = _ws_root(workspace_id)

    channel_types = ChannelRegistry.list_channels()
    channels = [_build_channel_info(ct, ws) for ct in sorted(channel_types)]
...
async def get_registered_channels(workspace_id: str):
    ...
    channels = [
        ChannelInfo(
            channel_type=ct,
            registered=True,
            enabled=_is_channel_enabled(ct, ws),
            running=_is_channel_running(ct),
            description=_get_channel_description(ct),
        )
...
async def health_check_all(workspace_id: str):
    ...
    for ct in channel_types:
        ...
        elif _is_channel_enabled(ct, ws):
...
async def get_channel_detail(workspace_id: str, channel_type: str):
    ...
    return _build_channel_info(channel_type, ws)
...
async def configure_channel(
    ...
    ws = _ws_root(workspace_id)
    try:
        _update_saved_config(channel_type, request.config, enabled=request.enabled, workspace_root=ws)
        logger.info(
...
async def enable_channel(workspace_id: str, channel_type: str):
    ...
    _set_channel_enabled(channel_type, True, _ws_root(workspace_id))
...
async def disable_channel(workspace_id: str, channel_type: str):
    ...
    _set_channel_enabled(channel_type, False, _ws_root(workspace_id))
