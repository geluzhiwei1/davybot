...
Config 按工作区隔离：``{ws}/.dawei/configs/acp_agents.json``（首次访问从全局懒 seed）。
全局 ``{DAWEI_HOME}/configs/acp_agents.json`` 作为用户级 seed 源（/api/users/me/acp-agents 管）。
"""
...
async def get_acp_agents(workspace_id: str):
    """获取所有已注册的 ACP agent 列表（per-workspace）"""
    agents = load_registry(_ws_root(workspace_id))
...
async def get_available_acp_agents(workspace_id: str):
    """获取当前可用的 ACP agent（已安装且未禁用，per-workspace）"""
    agents = list_available_agents(_ws_root(workspace_id))
    return ACPAgentsResponse(success=True, agents=agents)


@router.post("/scan", response_model=ACPScanResponse)
async def scan_acp_agents(workspace_id: str):
    """扫描系统 PATH，发现可用的 ACP agent 并与已有注册合并（per-workspace）"""
    try:
        agents = discover_and_merge(_ws_root(workspace_id))
...
async def create_acp_agent(workspace_id: str, request: ACPAgentAddRequest):
    """手动添加 ACP agent（per-workspace）"""
    try:
        agent = add_agent(
            command=request.command,
            name=request.name,
            description=request.description,
            workspace_root=_ws_root(workspace_id),
        )
...
async def delete_acp_agent(workspace_id: str, command: str):
    """删除 ACP agent（per-workspace）"""
    removed = remove_agent(command, _ws_root(workspace_id))
    if not removed:
...
async def toggle_acp_agent(workspace_id: str, command: str, request: ACPAgentToggleRequest):
    """启用/禁用 ACP agent（per-workspace）"""
    agent = toggle_agent(command, request.disabled, _ws_root(workspace_id))
    if not agent:
        raise HTTPException(status_code=404, detail=f"Agent '{command}' 不存在")
