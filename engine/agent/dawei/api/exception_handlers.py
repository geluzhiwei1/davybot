...
# Origins that must always receive CORS headers on error responses.
# Defensive: ensures Tauri/Browser never blocks error responses due to missing CORS.
# E3: 云端域名不再内置;额外源经 DAWEI_CORS_ORIGINS(逗号分隔)显式注入,与 server_app CORS 同源。
...
def _add_cors_headers(request: Request, response: JSONResponse) -> JSONResponse:
    """Add CORS headers to error responses as a safety net.

    Starlette CORSMiddleware should handle this, but certain edge cases
    (BrokenPipeError, middleware ordering) can cause it to skip headers.
    This ensures Tauri/Browser clients always receive valid CORS on errors.
    """
