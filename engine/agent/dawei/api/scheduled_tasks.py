...
def _validate_task_fields(task: Dict[str, Any]) -> None:
    ...
    # 验证 trigger_time 格式
    try:
        datetime.fromisoformat(task["trigger_time"])
    except ValueError as e:
        raise HTTPException(
            status_code=400,
...
async def _create_task_in_workspace(workspace_id: str, workspace_path: str, task: Dict[str, Any]) -> Dict[str, Any]:
    """在指定工作空间中创建定时任务（内部函数，供多处复用）
...
async def create_scheduled_task(workspace_id: str, task: Dict[str, Any]):
    ...
    """
    workspace_path = await _get_workspace_path(workspace_id)
    return await _create_task_in_workspace(workspace_id, workspace_path, task)
