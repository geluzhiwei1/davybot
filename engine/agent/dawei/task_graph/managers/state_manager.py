...
class StateValidator:
    ...
    # 定义允许的状态转换
    # CANCELLED(2026-09-17 P1a,与 task_validator.StatusTransitionValidationRule 同步):
    # 主动取消(用户终结/父 abort_task)终态;ABORTED 保留系统中止语义。
    # WAITING_FOR_TOOL/INTERACTIVE 不在此表(引擎先归一为 RUNNING 再终结)。
    VALID_TRANSITIONS: ClassVar[Dict[TaskStatus, set[TaskStatus]]] = {
        TaskStatus.PENDING: {TaskStatus.RUNNING, TaskStatus.ABORTED, TaskStatus.CANCELLED},
        TaskStatus.RUNNING: {
            TaskStatus.PAUSED,
            ...
            TaskStatus.FAILED,
            # P2-D: 崩溃残留恢复(task_graph_excutor._reconcile_subtasks_on_startup
            # RUNNING→PENDING 重置,此前被本表拒绝且被外层 except 吞掉——对账静默失效)
            TaskStatus.PENDING,
        },
        # P2-D: WAITING_FOR_TOOL 崩溃残留恢复允许直回 PENDING(终态目标仍缺席,
        # "引擎先归一为 RUNNING 再终结"的不变量不受影响)
        TaskStatus.WAITING_FOR_TOOL: {TaskStatus.PENDING},
        TaskStatus.PAUSED: {TaskStatus.RUNNING, TaskStatus.ABORTED, TaskStatus.CANCELLED},
        TaskStatus.COMPLETED: {
            # P2-D 重跑(2026-09-17): 显式用户/API 重跑动作可 COMPLETED→PENDING
            # (经 TaskGraph.reset_task_for_rerun 原位重置,先 stash result 再清空);
            # 常规执行路径仍锁终态——重置原语是唯一合法入口(不变量 6 不破)。
            TaskStatus.PENDING,
        },
        TaskStatus.FAILED: {
            # P2-D 重跑: 失败重试同上,显式重跑入口放开 FAILED→PENDING
            TaskStatus.PENDING,
        },
        TaskStatus.ABORTED: {
            # 中止状态通常是终态，但可以重启
        ...
        # CANCELLED 终态锁死（取消语义不可被后续路径改写，先到先得）。
        # 空集用 set() 而非 {}(空 dict 语义同,但与 task_validator 镜像表结构不可比)
    ...
    def is_terminal_state(self, status: TaskStatus) -> bool:
        """判断是否为终态（COMPLETED/FAILED/ABORTED/CANCELLED，与 TaskGraph._TERMINAL_STATUSES 一致）"""
        return status in {TaskStatus.COMPLETED, TaskStatus.FAILED, TaskStatus.ABORTED, TaskStatus.CANCELLED}
