...
class StatusTransitionValidationRule(ValidationRule):
    ...
    def __init__(self):
        ...
        # 定义允许的状态转换
        # CANCELLED(2026-09-17): 主动取消(用户终结/父 abort_task)终态;
        # ABORTED 保留为系统中止语义。WAITING_FOR_TOOL/INTERACTIVE 不在此表
        # (get() 回落空集=不可直接转换),由引擎先归一为 RUNNING 再终结
        self.valid_transitions = {
            ...
            TaskStatus.RUNNING: {
                ...
                TaskStatus.FAILED,
                # P2-D: 崩溃残留恢复 RUNNING→PENDING(与 StateValidator 同步)
                TaskStatus.PENDING,
            },
            ...
            # P2-D 重跑(2026-09-17,与 StateValidator.VALID_TRANSITIONS 同步):
            # COMPLETED/FAILED 经显式重跑入口(TaskGraph.reset_task_for_rerun)
            # 可回 PENDING(先 stash result 再清空,保住先到先得语义);
            # CANCELLED 仍锁死(用户终结语义不可改写)。
