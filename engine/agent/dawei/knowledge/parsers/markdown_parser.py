...
# Max file size for ingestion (50 MB) — prevents OOM on oversized files
...
class MarkdownParser(BaseParser):
    ...
    async def parse(self, file_path: str | Path) -> Document:
        ...
        # File size validation — prevent OOM on oversized files
        file_size = file_path.stat().st_size
        if file_size > MAX_FILE_SIZE_BYTES:
            raise ValueError(
                f"File too large: {file_size} bytes exceeds maximum {MAX_FILE_SIZE_BYTES} bytes"
            )
