...
class KnowledgeBaseManager:
    ...
    def get_fulltext_store(self, base_id: str):
        ...
        # NOTE: callers must `await store.initialize()` themselves (all current
        # callers do). Do NOT sync-initialize here — asyncio.run() /
        # run_until_complete() from this sync method raises inside the server's
        # already-running event loop, which previously broke full-text indexing
        # on first use (silently swallowed by upload's try/except, surfaced as
        # 500 by reindex).
    ...
    def list_base_documents(self, base_id: str, skip: int = 0, limit: int = 10) -> Dict[str, Any]:
        ...
        # ── 2. Fall back to fulltext.db when vectors.db has no docs ──
        if not documents_dict:
            fulltext_db_path = base_storage_path / "fulltext.db"
            if fulltext_db_path.exists():
                try:
                    with sqlite3.connect(str(fulltext_db_path)) as db:
                        cursor = db.cursor()
                        query = """
                            SELECT DISTINCT
                                document_id,
                                file_name
                            FROM fts_index
                            WHERE document_id IS NOT NULL AND file_name IS NOT NULL
                            ORDER BY rowid DESC
                        """
                        cursor.execute(query)
                        rows = cursor.fetchall()
                        ...
                        for row in rows:
                            ...
                            if document_id and file_name and document_id not in documents_dict:
                                uploads_dir = base_storage_path / "uploads"
                                uploaded_at = None
                                if uploads_dir.exists():
                                    file_path = uploads_dir / file_name
                    ...
                    logger.warning(f"Failed to query documents from fulltext database: {e}")
