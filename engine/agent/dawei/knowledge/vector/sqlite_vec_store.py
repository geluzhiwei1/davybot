...
# Whitelist of allowed metadata filter keys for workspace-isolated search.
# Filter keys are interpolated into json_extract() paths — only alphanumeric
# + underscore keys are safe. Other keys are rejected at query time.
_ALLOWED_FILTER_KEYS: set[str] | None = None  # None = validate pattern, set = exact match


def _validate_filter_key(key: str) -> str:
    """Validate a metadata filter key is safe for json_extract() interpolation."""
    if _ALLOWED_FILTER_KEYS is not None and key not in _ALLOWED_FILTER_KEYS:
        raise ValueError(f"Filter key not allowed: {key}")
    if not re.fullmatch(r"[a-zA-Z_][a-zA-Z0-9_]*", key):
        raise ValueError(f"Invalid filter key: {key} (only alphanumeric + underscore)")
    return key
