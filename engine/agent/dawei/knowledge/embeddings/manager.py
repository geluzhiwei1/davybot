...
# Resource limits to prevent OOM / API abuse
MAX_EMBEDDING_TEXT_LENGTH = 8192  # max chars per individual text
MAX_EMBEDDING_TOTAL_CHARS = 500_000  # max total chars across all texts
MAX_EMBEDDING_TEXT_COUNT = 1000  # max number of texts per request
...
def _get_embedding_api_config() -> tuple[str, str, str]:
    ...
    try:
        ...
        # Embedding fields live on the nested LLMProviderConfig (settings.llm),
        # NOT the top-level Settings — reading them flat always missed and fell
        # back to the default, which made the embedding model effectively
        # unconfigurable via env/.env.
        llm = get_settings().llm
        base_url = llm.llm_embedding_base_url or DEFAULT_OLLAMA_BASE_URL
        api_key = llm.llm_embedding_api_key or DEFAULT_OLLAMA_API_KEY
        model = llm.llm_embedding_model or DEFAULT_EMBEDDING_MODEL
...
class EmbeddingManager:
    ...
    async def embed(self, texts: List[str], batch_size: int = 32) -> List[List[float]]:
        ...
        Raises:
            ValueError: if input exceeds resource limits
        """
        if not texts:
            return []

        # Resource limit validation — prevent OOM / API abuse
        if len(texts) > MAX_EMBEDDING_TEXT_COUNT:
            raise ValueError(
                f"Too many texts: {len(texts)} exceeds maximum {MAX_EMBEDDING_TEXT_COUNT}"
            )
        total_chars = sum(len(t) for t in texts)
        if total_chars > MAX_EMBEDDING_TOTAL_CHARS:
            raise ValueError(
                f"Total text too large: {total_chars} chars exceeds maximum {MAX_EMBEDDING_TOTAL_CHARS}"
            )
        for i, text in enumerate(texts):
            if len(text) > MAX_EMBEDDING_TEXT_LENGTH:
                raise ValueError(
                    f"Text {i} too long: {len(text)} chars exceeds maximum {MAX_EMBEDDING_TEXT_LENGTH}"
                )
