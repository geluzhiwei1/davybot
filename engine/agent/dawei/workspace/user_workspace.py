...
# Security: environment variable prefixes that must NOT leak into
# task contexts, subprocess environments, or agent-facing metadata.
...
def _safe_env_copy() -> dict[str, str]:
    """Return os.environ with sensitive keys stripped.

    Used to prevent API keys, passwords, and tokens from leaking
    into TaskContext metadata visible to agents/tools.
    """
    env = os.environ.copy()
...
class UserWorkspace:
    ...
    def __init__(self, workspace_path: str):
        ...
        # 运行时用户身份（多租户隔离用；默认 default_user，由 chat handler 注入真实值）
        ...
        self._conversation_lock = asyncio.Lock()  # prevents race condition on concurrent conversation saves
        ...
        # mcp_tool_manager 通过 @property 代理到共享 context（见下方定义）
        # （P3-1 AgentProfile 工具面 overlay 已随 mode-工具解耦 D7 退役：
        #   子任务 allow/deny 仅由执行期守卫 tool_message_handler 强制）
        ...
        # ModeRegistry 单例句柄（mode统一管理.md §4.4：mode 唯一读入口）
        self._mode_registry = None
    ...
    # ── user_id：多租户隔离的运行时身份 ──────────────────────────────
    # 默认 "default_user"（local 桌面模式）；chat handler / websocket 在建立会话时
    # 通过 user_id setter 注入真实值，使 SecurityManager 按 user_id 分键隔离。
    @property
    ...
    def user_id(self, value: str | None) -> None:
        if value and value != self._user_id:
            if self._initialized and self._context is not None:
                # initialize() 后改身份不会换 context（context 按 (path, 旧 user_id)
                # 分键）——实例与所持 context 身份拆分,MCP/relay 等账户隔离资源
                # 将取错面。必须在 initialize() 前注入（2026-09-18 paper-search
                # 回归根因）；此处仅大声告警,不静默换键。
                logger.warning(
                    f"UserWorkspace.user_id 在 initialize() 之后才变更为 '{value}'"
                    f"（context 仍按 '{self._user_id}' 分键）——请在 initialize() 前注入。"
    ...
    def mcp_tool_manager(self):
        """MCP 工具管理器（从共享 context 获取，使 CRUD reload 对所有 Agent 生效）"""
        if hasattr(self, "_mcp_tool_manager_ref") and self._mcp_tool_manager_ref:
            return self._mcp_tool_manager_ref
        if self._context:
            return self._context.mcp_tool_manager
        return None

    @mcp_tool_manager.setter
    def mcp_tool_manager(self, value):
        """设置 MCP 工具管理器（用于向后兼容）"""
        self._mcp_tool_manager_ref = value
    ...
    def is_command_allowed(self, command: str) -> bool:
        """检查命令是否允许执行（详情版见 command_denial_reason）

        语义：
        1. Super mode → 直接放行
        2. 白名单关闭 → 直接放行
        3. 命中 custom_denied_commands → 拒绝
        4. 构建有效允许集合 = UNION(allowed_sources)
           - "system" in sources → 加入系统白名单所有命令名
           - "custom" in sources → 加入 custom_allowed_commands
        5. effective_allowed 为空 → 放行（白名单开但无限制）
        6. 命令名不在 effective_allowed → 拒绝
        7. "system" in sources → 调用 CommandWhitelist.validate_command() 做 flag/子命令验证
        ...
        Returns:
            bool: 命令是否在允许列表中
        """
        return self.command_denial_reason(command) is None

    def command_denial_reason(self, command: str) -> str | None:
        """返回命令被拒绝的具体原因（None = 允许）

        与 is_command_allowed 同一判定逻辑、同一日志，但把各分支的
        拒绝原因作为字符串返回，供 ToolSecurityError 透传给上层，
        避免 "is not allowed" 式黑盒报错。
        """
        # 1. Super mode
        ...
        logger.info(
            f"[is_command_allowed] command='{command[:50]}', "
            f"enable_command_whitelist={sec.get('enable_command_whitelist', True)}, "
            f"allowed_sources={sec.get('allowed_sources', ['system'])}"
        )
        ...
        denied_commands = sec.get("custom_denied_commands", [])
        if command_name in denied_commands:
            logger.debug(f"is_command_allowed: '{command_name}' in custom_denied_commands")
            return f"命令 '{command_name}' 在工作区自定义拒绝列表(custom_denied_commands)中"

        # 4. 构建有效允许集合（UNION of enabled sources）
        allowed_sources = sec.get("allowed_sources", ["system"])
        effective_allowed: set[str] = set()

        if "system" in allowed_sources:
            from dawei.sandbox.command_whitelist import CommandWhitelist

            # 确保已加载 JSON 配置（会话级调用一次）
            if not CommandWhitelist._loaded:
                CommandWhitelist.load_from_config()
            effective_allowed.update(CommandWhitelist.list_allowed_commands())

        if "custom" in allowed_sources:
            effective_allowed.update(sec.get("custom_allowed_commands", []))

        # 5. effective_allowed 为空 → 放行（白名单开但无限制）
        if not effective_allowed:
            logger.debug("is_command_allowed: no allowlist sources enabled, allowing all")
            return None

        # 6. 命令名不在 effective_allowed → 拒绝
        if command_name not in effective_allowed:
            logger.debug(f"is_command_allowed: '{command_name}' not in effective allowlist")
        ...
        if "system" in allowed_sources:
            ...
            if not is_valid:
                ...
                return f"系统白名单校验失败: {error_msg}"

        return None

    @property
    def mode(self):
        """当前mode（P0-1：未经显式设置时经 ModeResolver 兜底，绝不返回 None）

        背景（E2E 2026-09-18 综述流水线）：current_mode 为 None 时，prompt 用
        字面量 "orchestrator" 掩盖、task_node_executor 用 `or "pdca"` 兜底，
        两处口径不一，且自定义编排模式的 roleDefinition 从未激活。统一经
        resolve 链（explicit > task > mention > orchestrator > pdca）收口。
        """
        if not self.workspace_info or not self.workspace_info.user_ui_context:
            return None
        current = self.workspace_info.user_ui_context.current_mode
        if current:
            return current
        try:
            from dawei.mode.registry import ModeSelectionContext

            return self.mode_registry.resolve(ModeSelectionContext())
        except Exception:  # noqa: BLE001 — registry 不可用时退内置兜底，不阻断读取
            logger.warning("mode fallback: registry resolve failed, using 'orchestrator'")
            return "orchestrator"
    ...
    def mode_manager(self):
        """延迟初始化 ModeManager（Phase 4：委托 ModeRegistry 持有的 manager —
        invalidate_registry 后自动跟随新实例，无陈旧窗口）"""
        mm = self.mode_registry.manager
        if self._mode_manager is not mm:
            self._mode_manager = mm
        return self._mode_manager

    @property
    def mode_registry(self):
        """ModeRegistry per-workspace 单例（mode统一管理.md §4.4：mode 唯一读入口）

        所有消费方（switch_mode、mode_utils、API 列表、工具准入）一律经此读取，
        禁止再 ad-hoc ModeManager(...)（P6）。Phase 4 invalidate 挂钩：每次取单例
        对象做同一性校验，被 resource_installer 装后失效重建后自动跟随。
        """
        from dawei.mode.registry import get_registry

        current = get_registry(self.absolute_path)
        if current is not self._mode_registry:
            self._mode_registry = current
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self._context = await WorkspaceService.get_context(self.absolute_path, self.user_id)
            logger.info(f"  ✓ Acquired WorkspaceContext (refs={self.context.ref_count})")

            # 通知 SecurityManager 当前上下文（user_id + workspace）
            from dawei.core.security_manager import security_manager
            security_manager.set_context(self.user_id, self.absolute_path)
            ...
            if self.workspace_settings and self.workspace_settings.security:
                security_manager.update_workspace_security(
                    self.workspace_settings.security,
                    user_id=self.user_id,
                    workspace_id=self.absolute_path,
                )
    ...
    async def get_settings(self) -> dict:
        ...
        if self.workspace_settings:
            ...
            settings["llm_model"] = self.llm_manager.get_current_config_name()
    ...
    async def save_current_conversation(self) -> bool:
        ...
        async with self._conversation_lock:
            return await self._save_current_conversation_locked()

    async def set_and_save_conversation(self, conversation: Conversation) -> None:
        """原子地设置当前对话并保存，避免竞态条件"""
        async with self._conversation_lock:
            self.current_conversation = conversation
            await self._save_current_conversation_locked()

    async def save_subtask_conversation(self, conversation: Conversation) -> bool:
        """保存子任务独立会话（P2-7）：不切换 current_conversation 指针。

        复用 _save_current_conversation_locked 的持久化底层，按 conversation.id
        落盘 —— 重启后可按 TaskData.conversation_id 恢复子会话内容。
        """
        if conversation is None:
            return False
        async with self._conversation_lock:
            return await self._save_current_conversation_locked(conversation)

    async def _save_current_conversation_locked(self, conversation: Conversation | None = None) -> bool:
        """内部方法：执行实际保存（调用者必须持有 _conversation_lock）

        Args:
            conversation: 要保存的会话；None = 保存 current_conversation（默认）
        """
        conv = conversation if conversation is not None else self.current_conversation
        ...
        logger.info(
            f"[SAVE_CONVERSATION] Conversation details: id={conv.id}, message_count={conv.message_count}",
        )

        # 首先尝试使用持久化管理器保存
        if self.persistence_manager:
            # 将对话转换为字典格式
            conversation_dict = self._conversation_to_dict(conv)
            ...
            if success:
                logger.info(
                    f"[SAVE_CONVERSATION] Successfully saved conversation via persistence manager: {conv.id}",
                )
                return True
            logger.warning(
        ...
        if success:
            logger.info(
                f"[SAVE_CONVERSATION] Successfully saved conversation via history manager: {conv.id}",
            )
        else:
            logger.error(
                f"[SAVE_CONVERSATION] Failed to save conversation: {conv.id}",
            )
            # 发送持久化失败告警
            await self._alert_persistence_failure(
                "conversation",
                f"Failed to save conversation {conv.id}",
    ...
    def _conversation_to_dict(self, conversation: Conversation) -> Dict[str, Any]:
        ...
        return {
            ...
            "source_task_id": conversation.source_task_id,
    ...
    def allowed_tools(self) -> List[Dict[str, Any]]:
        ...
        if self.tool_manager:
            ...
            # 关闭时不可见也不可执行
            mcp_dynamic = self._mcp_dynamic_tools()
            if mcp_dynamic:
    ...
    def _mcp_dynamic_tools(self) -> List[Dict[str, Any]]:
        """已连接 MCP server 的工具 → 一级工具条目（详见 mcp_dynamic_tools 模块注释）。

        失败 FAST FAIL 返回空列表，不阻断静态工具供给。
        """
        try:
            from dawei.tools.mcp_dynamic_tools import build_mcp_tool_dicts

            return build_mcp_tool_dicts(self.mcp_tool_manager)
        except Exception:  # noqa: BLE001
            logger.exception("build first-class MCP tools failed")
            return []

    def _get_filtered_tool_names(self, tools: List[Dict[str, Any]]) -> set[str]:
        """Filter tool names according to workspace settings.

        Delegates to ToolManager.get_filtered_tool_names() when available.

        SECURITY: The fallback path (when ToolManager is unavailable) uses
        default-allow semantics — any tool that doesn't match explicit MCP
        or browser checks is automatically permitted (§ line 1529-1530).
        This means potentially dangerous tools (e.g. execute_command, shell,
        file_write) pass through the filter if no workspace_settings
        restrictions are configured.  When deploying to multi-user or
        untrusted environments, ensure workspace_settings are properly
        populated OR ToolManager is always available.
        """
        if self.tool_manager:
            return self.tool_manager.get_filtered_tool_names(tools, self.workspace_settings)
        # Fallback: basic filtering when ToolManager is unavailable.
        # NOTE: this path allows all tools by default (default-allow).
        ...
        for tool in tools:
            ...
            # Basic tools are always permitted
            if tool_name in [
                "read_file",
                "write_text_file",
                "list_files",
            ...
            # MCP tools require workspace_settings.always_allow_mcp
            if tool_name.startswith(("mcp_", "use_mcp_")):
                if self.workspace_settings.always_allow_mcp:
                    allowed_tools.add(tool_name)
                continue

            # Browser tools require workspace_settings.always_allow_browser
            if "browser" in tool_name.lower() or "chrome" in tool_name.lower():
                if self.workspace_settings.always_allow_browser:
                    allowed_tools.add(tool_name)
                continue

            # Default-allow: any tool not explicitly filtered above
            # is permitted.  This is intentional for single-user desktop
            # use but should be reviewed for multi-tenant deployments.
            allowed_tools.add(tool_name)
    ...
    def is_path_allowed(self, path: str) -> bool:
        ...
        # 文件系统策略（域4）：blocked_paths（override-or-inherit）。
        # 中心点 —— 所有文件工具经 is_path_allowed，一处即全覆盖；策略读取失败永不阻断。
        try:
            from dawei.core.security_manager import security_manager

            blocked = security_manager.get_policy().filesystem.blocked_paths
            if blocked:
                abs_lower = str(abs_path).lower()
                for prefix in blocked:
                    # 规范化前缀（与已 resolve 的 abs_path 一致），兼容 ~ 与大小写/短名差异
                    try:
                        norm = str(Path(prefix).expanduser().resolve()).lower()
                    except Exception:
                        norm = str(prefix).lower()
                    if abs_lower.startswith(norm):
                        try:
                            from dawei.core.security_auditor import security_auditor

                            security_auditor.log(
                                "security.filesystem.denied",
                                path=str(abs_path),
                                reason="blocked_paths",
                            )
                        except Exception:
                            pass
                        logger.warning(f"is_path_allowed: '{abs_path}' blocked by security policy")
                        return False
        except Exception:
            pass
    ...
    async def create_or_restore_task_graph(self):
        ...
        return TaskGraph(task_id="default", workspace_id=self.workspace_info.id if self.workspace_info else None, event_bus=None)
