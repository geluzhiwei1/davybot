...
class WorkspaceContext:
    ...
    def __init__(self, workspace_path: str, user_id: str = "default_user"):
        self.workspace_path = Path(workspace_path).resolve()
        self.absolute_path = str(self.workspace_path)
        self.user_id = user_id or "default_user"
        # 多用户：上下文按 (path, user_id) 唯一标识。同 path 不同 user 得独立 context
        # （各自的 LLMProvider/MCPToolManager/会话历史/安全上下文，加载各自的用户级配置）。
        self.workspace_id = f"{self.absolute_path}::{self.user_id}"
    ...
    @staticmethod
    def get_id_from_path(workspace_path: str) -> str:
        """从路径获取 resolved 路径字符串（向后兼容）。"""
        return str(Path(workspace_path).resolve())

    @staticmethod
    def get_context_key(workspace_path: str, user_id: str = "default_user") -> tuple[str, str]:
        """获取 (resolved_path, user_id) 二元组键——多用户隔离的唯一标识。"""
        return (str(Path(workspace_path).resolve()), user_id or "default_user")
    ...
    async def initialize(self):
        ...
        async with self._lock:
            ...
            try:
                ...
                # 1.5 初始化共享 MCPToolManager（须早于 ToolManager：MCP 工具类实例化时
                # ctx 尚未注册进 _contexts，靠进程级注册表 get_or_create_mcp_manager()
                # 保证工具类 fallback 拿到的是同一个实例，避免 connect/use 状态分裂）。
                # 多用户：传 user_id，使其加载该用户的用户级 mcp 配置。
                from dawei.tools.mcp_tool_manager import get_or_create_mcp_manager

                self.mcp_tool_manager = get_or_create_mcp_manager(
                    workspace_root=self.absolute_path, user_id=self.user_id
                )
                logger.info("  ✓ MCPToolManager initialized (shared, process-registry backed)")

                # 2. 初始化工具管理器（4层配置加载）。多用户：传 user_id 供 MCP 工具链使用。
                ...
                # 3. 初始化 LLM 管理器。多用户：传 user_id，加载该用户的用户级 LLM providers。
                self.llm_manager = LLMProvider(
                    workspace_root=self.absolute_path, user_id=self.user_id
                )
    ...
    async def _cleanup(self):
        ...
        try:
            ...
            # 4.5 清理共享 MCP 管理器（断开所有已连接 server 子进程）
            if self.mcp_tool_manager:
                try:
                    await self.mcp_tool_manager.disconnect_all_servers()
                    logger.info("  ✓ MCPToolManager stopped")
                except Exception:
                    logger.exception("  ✗ Error stopping MCPToolManager: ")
...
class WorkspaceService:
    ...
    # 类变量：所有工作区上下文（key = (resolved_path, user_id) 二元组）
    _contexts: ClassVar[Dict[tuple[str, str], WorkspaceContext]] = {}
    _lock: ClassVar[asyncio.Lock] = asyncio.Lock()

    @classmethod
    async def get_context(
        cls, workspace_path: str, user_id: str = "default_user"
    ) -> WorkspaceContext:
        """获取工作区上下文（每个 (workspace_path, user_id) 一个单例）。

        多用户：同 path 不同 user 得独立 context（各自的 LLM/MCP/会话/安全上下文）。
        """
        key = WorkspaceContext.get_context_key(workspace_path, user_id)

        # 检查是否已存在
        logger.debug(f"get_context() called for {key}")
        logger.debug(f"  Existing contexts: {list(cls._contexts.keys())}")

        if key in cls._contexts:
            ...
            if context is not None and context.is_initialized():
                ...
                logger.debug(f"Reusing existing WorkspaceContext: {key} (refs={context.ref_count})")
                return context
        ...
        async with cls._lock:
            ...
            if key in cls._contexts:
                ...
                if context is not None and context.is_initialized():
                    await context.retain()
                    logger.debug(f"Reusing existing WorkspaceContext (after lock): {key}")
                    return context

            # 创建并初始化新上下文
            logger.info(f"Creating new WorkspaceContext: {key}")
            context = WorkspaceContext(workspace_path, user_id)
            ...
            logger.info(f"✓ New WorkspaceContext registered: {key} (refs={context.ref_count})")
            return context

    @classmethod
    async def remove_context(cls, workspace_path: str, user_id: str = "default_user"):
        """强制移除工作区上下文（谨慎使用，用于工作区删除等场景）。"""
        key = WorkspaceContext.get_context_key(workspace_path, user_id)
        async with cls._lock:
            if key in cls._contexts:
                context = cls._contexts[key]
                ...
                logger.info(f"WorkspaceContext removed: {key}")

    @classmethod
    async def invalidate_context(cls, workspace_path: str) -> int:
        """使某工作区的**全部** WorkspaceContext 缓存失效（不清理在用实例）。

        【2026-09-14 安装后生效】market 资源安装后调用: 已初始化 context 的
        ToolManager/ModeManager/MCPToolManager 都是安装前的快照, 不失效则新装的
        skills/agents/modes/mcp 要重启进程才可见。与 remove_context 不同, 这里
        只从注册表摘除 key —— 在用会话继续持有旧 context 正常收尾
        (release → _cleanup), 下一次 get_context 会重建并加载新资源。
        ...
        removed = 0
        async with cls._lock:
            for key in list(cls._contexts.keys()):
                if key[0] == target:
                    del cls._contexts[key]
                    removed += 1
        if removed:
            logger.info(
                "WorkspaceContext invalidated for %s (%d context(s)); next access rebuilds",
                target, removed,
            )
        return removed

    @classmethod
    def get_all_contexts(cls) -> Dict[tuple[str, str], WorkspaceContext]:
        """获取所有活跃的工作区上下文（key=(path, user_id)，用于监控/reload）。"""
        return dict(cls._contexts)

    @classmethod
    def get_context_if_initialized(
        cls, workspace_path: str, user_id: str = "default_user"
    ) -> WorkspaceContext | None:
        """同步获取已初始化的工作区上下文（不创建、不 retain）。

        供 MCP 工具类 __init__ 等同步场景按 (path, user_id) 查找共享 mcp_tool_manager。
        找不到返回 None。
        """
        try:
            key = WorkspaceContext.get_context_key(workspace_path, user_id)
            ctx = cls._contexts.get(key)
            if ctx is not None and ctx.is_initialized():
                return ctx
        except Exception:
            pass
        return None
