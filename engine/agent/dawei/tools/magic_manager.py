...
# Security: environment variable prefixes to strip from magic command subprocesses
_MAGIC_SENSITIVE_ENV_PREFIXES = (
...
def _magic_safe_env() -> dict[str, str]:
    """Return os.environ copy with sensitive keys stripped."""
    env = os.environ.copy()
    for key in list(env.keys()):
        if key.upper().startswith(_MAGIC_SENSITIVE_ENV_PREFIXES):
            del env[key]
    return env
...
class MagicContext:
    ...
    SECURITY: The ``env`` field is a filtered copy of os.environ.
    Subprocess execution (``!``, ``%%bash``, ``%time``) uses
    ``shell=True`` — do NOT expose this interface to untrusted
    remote clients without additional sandboxing.
    """

    cwd: Path = field(default_factory=Path.cwd)
    env: Dict[str, str] = field(default_factory=_magic_safe_env)
...
class MagicCommandManager:
    ...
    def _magic_cell_python(self, _args: str, content: str, context: MagicContext) -> str:
        ...
        SECURITY: exec() uses a restricted builtins namespace to prevent
        access to ``open()``, ``__import__``, ``eval()``, and other
        dangerous builtins.  Only safe builtins (print, range, len, etc.)
        are available.  This is a best-effort sandbox — do NOT expose
        %%python to untrusted remote clients.
        ...
        # Restricted builtins whitelist — prevents access to open(),
        ...
        _SAFE_BUILTINS = {
            ...
            "zip": zip, "complex": complex, "True": True, "False": False,
        ...
        try:
            exec(content, {"__builtins__": _SAFE_BUILTINS, "context": context})
