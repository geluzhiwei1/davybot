...
class SmartFileEditInput(BaseModel):
    ...
    edits: List[Dict[str, str]] = Field(
        ...,
        description=(
            "List of edits to apply. Each edit must have 'search' and 'replace' fields "
            "(aliases 'old_string'/'new_string' are also accepted)."
        ),
    )
...
**Format:**
edits = [
  {"search": "old section", "replace": "new section"},      # or
  {"old_string": "old part", "new_string": "new part"}      # alias form
]
...
- Order edits logically (top to bottom of file)"""
    ...
    def _run(self, file_path: str, edits: List[Dict[str, str]], chunk_size: int = 50) -> str:
        ...
        # Validate edits（归一化别名：LLM 常用 old_string/new_string，见 2026-09-17 事故）
        if not edits:
            return "Error: No edits provided"

        normalized_edits: List[Dict[str, str]] = []
        for i, edit in enumerate(edits):
            search_text = edit.get("search", edit.get("old_string"))
            replace_text = edit.get("replace", edit.get("new_string"))
            if search_text is None or replace_text is None:
                return (
                    f"Error: Edit {i + 1} missing 'search'/'replace' "
                    f"(or 'old_string'/'new_string') field"
                )
            normalized_edits.append({"search": search_text, "replace": replace_text})
