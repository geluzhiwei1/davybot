...
class InsertContentTool(CustomBaseTool):
    ...
    def _run(self, path: str, line: int, content: str) -> str:
        ...
        if workspace_dir:
            ...
            # Security: without a workspace boundary, reject dangerous paths
            resolved = Path(path).resolve()
            _danger = {"/etc", "/sys", "/proc", "/dev", "/root", "/var", "/boot"}
            if any(str(resolved).startswith(d) for d in _danger) or ".." in str(path):
                return (
                    "Error: File write outside workspace boundaries is not allowed. "
