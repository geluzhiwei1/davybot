...
class DocxReadStructuredTool(CustomBaseTool):
    ...
    def _run(
        ...
        try:
            ...
            # inner-layer governance: cap total output size
            _MAX_OUTPUT = 30000  # ~30KB, ~7.5K tokens
            if len(result) > _MAX_OUTPUT:
                result = result[:_MAX_OUTPUT] + f"\n\n⚠️ 内容已截断（共 {len(result)} 字符，显示前 {_MAX_OUTPUT}）。使用 start_paragraph/end_paragraph 分段读取。"
