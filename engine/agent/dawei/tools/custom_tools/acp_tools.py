...
class CallACPAgentTool(CustomBaseTool):
    ...
    def _run(
        ...
        # Validate command against registry（per-workspace：工具运行时已注入 user_workspace）
        ws_root = str(self.user_workspace.absolute_path) if self.user_workspace else None
        available = list_available_agents(ws_root)
