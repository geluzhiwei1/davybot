...
class TaskExecutionLock:
    ...
    async def acquire(self, task_id: str) -> bool:
        ...
        async with self._global_lock:
            ...
            self._lock_timestamps[task_id] = time.monotonic()
    ...
    async def release(self, task_id: str) -> None:
        ...
        async with self._global_lock:
            if task_id in self._locks:
                ...
                # 清理: 只移除未锁定且最旧的锁, 避免误删活跃锁
                if len(self._locks) > 100:
                    unlocked = [
                        tid for tid, lck in self._locks.items()
                        if not lck.locked() and tid != task_id
                    ]
                    unlocked.sort(key=lambda tid: self._lock_timestamps.get(tid, 0))
                    for tid in unlocked[:50]:
                        del self._locks[tid]
                        self._lock_timestamps.pop(tid, None)
...
class SchedulerEngine:
        ...
        while self._running:
            try:
                ...
                if now - last_cleanup > cleanup_interval:
                    await self.storage.cleanup_old_tasks()
                    last_cleanup = now
    ...
    async def _execute_message_task(self, task: ScheduledTask) -> None:
        ...
        Args:
            task: 定时任务
        """
        # 自主执行策略（域9）：门控定时任务是否可创建全权 Agent。
        # 策略读取异常被吞（放行），不破坏既有调度流程。
        try:
            from dawei.entity.scheduled_task import ScheduleType

            from dawei.core.security_manager import security_manager

            autonomy = security_manager.get_policy().autonomy
            is_cron = task.schedule_type == ScheduleType.CRON
            if not autonomy.allow_scheduled_tasks or (is_cron and not autonomy.allow_cron):
                try:
                    ...
                    security_auditor.log(
                        ...
                        reason=(
                            ...
                            if not autonomy.allow_scheduled_tasks
                            else "cron_disabled"
                        ),
                        schedule_type=getattr(task.schedule_type, "value", str(task.schedule_type)),
                ...
                raise PermissionError(
                    "Scheduled task execution blocked by autonomy security policy"
                )
        except PermissionError:
            raise
        except Exception as e:
            logger.warning(f"Autonomy scheduler check failed: {e}; allowing")
        ...
        # 注入工作区归属账号（initialize() 之前,context 按 (path, user_id) 分键）
        owner_uid = workspace_info.get("owner_user_id")
        if owner_uid:
            user_workspace.user_id = owner_uid
        ...
        # 记录 workspace 上下文到 task metadata
        if task.metadata is None:
            task.metadata = {}
        task.metadata["last_execution_conversation_id"] = virtual_session_id
        task.metadata["last_execution_at"] = datetime.now(UTC).isoformat()
...
class SchedulerManager:
    ...
    async def list_all_tasks(self, status: str | None = None) -> List[ScheduledTask]:
        """获取所有 workspace 的任务列表
        ...
        async with self._lock:
            ...
            for engine in self._engines.values():
                tasks = await engine.storage.list_tasks()
                if status:
                    tasks = [t for t in tasks if t.status.value == status]
                all_tasks.extend(tasks)

        # 再从未加载的 workspace 获取 (不加全局锁, 避免长时间持锁)
        loaded_ids = set(self._engines.keys())
        all_workspaces = workspace_manager.get_all_workspaces()
        for ws in all_workspaces:
            ws_id = ws.get("id")
            ws_path = ws.get("path")
            if not ws_id or not ws_path or ws_id in loaded_ids:
                continue
