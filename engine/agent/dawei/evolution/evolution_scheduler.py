...
class EvolutionScheduler:
    ...
    async def _get_or_init_workspace(self, workspace_path: str, workspace_info: dict | None = None):
        ...
        Args:
            ...
            workspace_info: 注册表信息（注入 owner_user_id,账户隔离用）
        ...
        # 注入归属账号（initialize() 之前,context 按 (path, user_id) 分键）
        owner_uid = (workspace_info or {}).get("owner_user_id")
        if owner_uid:
    ...
    async def _check_all_workspaces(self):
        ...
        for workspace_info in all_workspaces:
            ...
            workspace = await self._get_or_init_workspace(workspace_path, workspace_info)
