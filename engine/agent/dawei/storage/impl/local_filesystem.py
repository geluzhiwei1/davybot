...
# Directories that should always be excluded from file listings
# .dawei contains temporary/workspace config files — never show these to users
_EXCLUDED_DIR_NAMES: Set[str] = {".dawei"}
    ...
    async def list_directory(
        ...
        def _walk(current_path: Path, current_depth: int):
            ...
            for entry in current_path.iterdir():
                # Always skip hidden files/dirs unless explicitly included
                if not include_hidden and entry.name.startswith("."):
                    continue
                # Always skip excluded directory names (e.g. .dawei) and their contents
