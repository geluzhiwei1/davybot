...
class AsyncTaskManager(IAsyncTaskManager):
    ...
    async def _execute_task(self, task_id: str) -> None:
        ...
        result = None
        task_result: TaskResult | None = None  # finally 中兜底判断用
        ...
        try:
            ...
            self._stats["running_tasks"] -= 1
            # 不再 re-raise（2026-09-18）：错误已通过 Future 完整交付给 wait_for_task；
            # _schedule_task 是 fire-and-forget create_task，re-raise 只会产生
            # "Task exception was never retrieved" 日志噪音，无人消费。
            ...
            # 兜底（2026-09-18）：异常发生在 task_result 赋值前（如 BaseException
            # 逃逸出 except 分支）时，构造 FAILED 结果，保证 Future 一定被 set_result，
            # 避免 wait_for_task 空等到 tool_timeout+30s 超时。
            if task_result is None:
                task_result = TaskResult(
                    task_id=task_id,
                    status=TaskStatus.FAILED,
                    error=RuntimeError(f"Task failed before result was built: {task_id}"),
                    execution_time=time.time() - start_time,
                    started_at=context.get_data("started_at"),
                    completed_at=datetime.now(UTC),
                )
