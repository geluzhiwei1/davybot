...
class WebSocketState:
    ...
    def is_healthy(self) -> bool:
        ...
        # 检查心跳是否超时。
        # 阈值必须与 WebSocketManager.connection_timeout(默认180s) 的设计意图一致：
        # 浏览器会把后台标签页的 setInterval 节流到约 1次/分钟（5分钟后），前端心跳
        # 间隔为 30s，被节流后会拉长到 ~60s+。若阈值仍是旧的 60s，后台/繁忙标签页的
        # 连接会被误判为不健康 → send_message 拒绝投递 → FOLLOWUP_QUESTION 等实时事件
        # 送不到前端（追问弹窗永不出现，直到追问自身 300s 超时）。这里放宽到 150s，
        # 既能容忍后台节流，又仍早于 180s 的回收阈值，留出回收窗口。
        import time

        try:
            return (time.time() - self.last_heartbeat.timestamp()) < 150
        except (AttributeError, TypeError):
            return False
