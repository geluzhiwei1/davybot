...
class DomainAdapter:
    ...
    def _init_domain_contexts(self) -> Dict[DomainType, DomainContext]:
        ...
        return {
            ...
            DomainType.COMPLIANCE: DomainContext(
                domain=DomainType.COMPLIANCE,
                typical_outputs=["合规分析报告", "风险评估报告", "筛查结果", "合规整改方案", "政策解读", "合规手册", "申报材料"],
                quality_criteria=["法规覆盖全面", "风险评估准确", "分析逻辑严谨", "建议可操作", "引用法规最新", "时效性合规"],
                common_tools=["法规数据库", "筛查工具", "风险评估矩阵", "文档生成器", "监控告警系统", "案例库", "MCP工具"],
                success_metrics=["风险识别率", "筛查准确率", "合规覆盖率", "整改完成率", "审计通过率"],
                workflows={
                    "plan": ["合规需求分析", "法规检索与研究", "风险评估框架设计", "制定合规计划"],
                    "do": ["执行合规筛查", "进行尽职调查", "准备合规文档", "实施风险控制", "开展合规培训"],
                    "check": ["结果验证与复核", "合规差距分析", "风险评估审核", "法规更新检查", "审计追踪"],
                    "act": ["优化合规流程", "更新合规政策", "培训团队成员", "总结经验教训", "预防措施制定"],
                },
            ),
    ...
    def detect_domain_from_context(self, context: Dict[str, Any]) -> DomainType:
        ...
        # 检查工具
        if any(t in tools_str for t in ["compliance", "sanction", "screening", "kyc", "cdd", "aml", "regulatory"]):
        ...
        if "data" in environment or "analytics" in environment:
            return DomainType.DATA_ANALYSIS
        if "compliance" in environment or "legal" in environment or "regulatory" in environment:
            return DomainType.COMPLIANCE

        # 检查文件路径中的合规相关文件
        for file_path in file_paths:
            if any(kw in file_path.lower() for kw in ["rule", "regulation", "compliance", "sanction", "policy", "audit"]):
                return DomainType.COMPLIANCE
