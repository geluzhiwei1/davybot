...
class LLMMCall:
    ...
    gateway_credits: float | None = None  # Gateway 模式下的实际积分消耗
...
class CostSummary:
    ...
    total_gateway_credits: float | None = None  # Gateway 模式下总积分消耗
...
class CostTracker:
    ...
    def record_gateway_credits(
        ...
        """记录 Gateway 模式下的积分消耗（替代 USD 成本）

        当 LLM 请求通过 Gateway 时，成本由积分系统而非 USD 定价决定。
        此方法将积分信息附加到最近一条调用记录上。
        """
        # 找到最近一条同模型的记录或创建新记录
        for call in reversed(self.calls):
            if call.model == model and call.gateway_credits is None:
                call.gateway_credits = credits
                self.logger.debug(
                    f"Gateway credits recorded: {model} — {credits:.2f} credits "
        ...
        # 如果没有找到匹配的记录，创建一条新记录
        self.logger.warning(
            f"No matching call found for gateway credits: {model} {credits:.2f}"
        )
    ...
    def get_summary(self) -> CostSummary:
        ...
        # Gateway 积分汇总
        gateway_credits = sum(
            c.gateway_credits for c in self.calls if c.gateway_credits is not None
        )
        total_gateway = round(gateway_credits, 4) if gateway_credits > 0 else None
        ...
        return CostSummary(
            ...
            total_gateway_credits=total_gateway,
        )
