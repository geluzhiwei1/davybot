...
class MessageValidationMiddleware:
    ...
    def __init__(
        ...
        #
        # NOTE: This middleware pattern-matches the *text* of user chat messages.
        # These are interpreted by the LLM (intent) and any real execution happens
        # through tools that have their own sandbox / command-whitelist validation
        # (see dawei/sandbox/). Chat input routinely and legitimately contains
        # backticks, ``$(...)``, tildes, version strings like ``1.2``, English
        # words like "executive", HTML entities, etc. So we keep ONLY high-signal
        # patterns that won't appear in ordinary legal/technical prose.
        #
        # Removed (over-broad, caused false positives on normal text):
        #   - command_injection entirely (`` `.*` ``, `` $(.*) ``, `` &&\w+ ``,
        #     `` ;ls `` …) — the agent's command sandbox already gates execution.
        #   - sql: `` \bexec\b `` (matches "executive"), `` ' ; ``, generic
        #     `` or … = … or `` / `` and … = … and `` (natural-language matches).
        #   - xss: `` onerror= ``/`` onload= ``/`` onclick= ``, `` &# ``/`` &#x ``
        #     (any HTML entity).
        #   - path_traversal: `` ~ `` (any tilde), `` \.\.[a-zA-Z0-9] `` (versions).
