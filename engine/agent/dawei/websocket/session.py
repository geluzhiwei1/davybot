...
class SessionData:
    ...
    v2 扩展 (§沙箱系统升级 v2 — Phase 3):
    - ``is_authenticated``: 由 JWT 认证层保证, user_id 非空即视为已认证
    - ``permissions``: 来自 JWT payload 的权限集, 用于 TrustedContext 构造
    - ``on_close()`` / ``on_reconnect()``: WebSocket 生命周期钩子, 透传到 SandboxFacade
    """
    ...
    # === v2: 认证与权限 ===
    # JWT 认证在 api/websocket.py 完成, SessionData 创建时 user_id 已经过 JWT 验证
    # permissions 存储在 metadata 中, 避免与 asdict() 序列化冲突

    @property
    def is_authenticated(self) -> bool:
        """是否已通过认证 — user_id 非空即视为已认证 (JWT 在 API 层验证)"""
        return self.user_id is not None

    @property
    def permissions(self) -> List[str]:
        """权限集 — 从 metadata 获取, 默认空集"""
        return self.metadata.get("permissions", [])

    @permissions.setter
    def permissions(self, value: List[str]) -> None:
        self.metadata["permissions"] = list(value)

    # === v2: WebSocket 生命周期钩子 (§5.3) ===

    async def on_close(self) -> None:
        """WebSocket 断开 — 通知沙箱进入 grace 期

        由 WebSocket manager disconnect 流程调用。
        不立即销毁沙箱, 由 Provider 的 grace 期机制处理。
        """
        ...
        try:
            ...
            ctx = from_user_workspace(
                user_id=self.user_id,
                workspace_path=self.workspace_id,
                permissions=self.permissions,
            )
            SandboxFacade.on_disconnect(ctx)
        except Exception as e:
            logger.warning(f"SessionData.on_close 沙箱通知失败: {e}")

    async def on_reconnect(self) -> None:
        """WebSocket 重连 — 通知沙箱恢复并排空队列

        由 WebSocket manager reconnect 流程调用。
        """
        if not self.user_id or not self.workspace_id:
            return
        ...
        try:
            ...
            SandboxFacade.on_reconnect(ctx)
        except Exception as e:
            logger.warning(f"SessionData.on_reconnect 沙箱通知失败: {e}")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # v2: permissions 已在 metadata 中, 由 asdict() 自动序列化
        return result
