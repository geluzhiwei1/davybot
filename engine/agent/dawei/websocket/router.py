...
class MessageRouter:
    ...
    async def route_message(self, session_id: str, message: BaseWebSocketMessage, message_id: str):
        ...
        if not handlers:
            # ws_heartbeat is a keepalive probe — it has no handler by design.
            # Sending a NO_HANDLER error for it every heartbeat (30s) is just
            # noise (and a wasted send the frontend drops anyway). Silently no-op.
            if is_heartbeat:
                return
