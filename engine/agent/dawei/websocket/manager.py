...
class ConnectionInfo:
    ...
    def __init__(self, websocket: WebSocket, session_id: str, workspace_id: str | None = None):
        # 验证输入
    ...
    def is_timeout(self, timeout_seconds: int = 60) -> bool:
        """检查连接是否超时"""
        return (datetime.now(UTC) - self.last_heartbeat).total_seconds() > timeout_seconds
...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, session_id: str, workspace_id: str | None = None) -> bool:
        """建立WebSocket连接

        Args:
            websocket: WebSocket连接对象
            session_id: 会话ID
            workspace_id: 工作空间ID（用于 broadcast 按 workspace 过滤）
        ...
        # 创建连接信息
        connection_info = ConnectionInfo(websocket, session_id, workspace_id=workspace_id)
    ...
    async def broadcast(
        ...
        Args:
            ...
            workspace_id: 若提供,仅广播给该 workspace 的连接（None = 不过滤）

        """
        exclude_sessions = exclude_sessions or set()
        failed_sessions = []

        for session_id, conn_info in list(self.active_connections.items()):
            if session_id in exclude_sessions:
                continue
            # 按 workspace 过滤：若指定了 workspace_id 且连接属于不同 workspace，跳过
            if workspace_id is not None and (conn_info.workspace_id or None) != workspace_id:
                continue
    ...
    async def _heartbeat_loop(self):
        ...
        while self._is_running:
            try:
                ...
                # 【2026-09-14 边缘 keepalive】服务端主动推送 ws_heartbeat:
                # 链路实测: 客户端 → 边缘 nginx(officeServer01, proxy_read_timeout
                # 86400s, 非切割点) → web02 dawei:8431。真正的切割点是本管理器
                # 自己的 180s 客户端静默回收 (_check_connections → "连接超时"):
                # 无心跳客户端(冒烟脚本/挂起浏览器标签)在长任务(沙箱冷启动+pip
                # ~72s, chat 总耗时 ~210s)期间被回收 → 客户端 "Connection to
                # remote host was lost", 服务端任务其实已完成。每 30s 推一条
                # ws_heartbeat (与客户端心跳同类型, 前端本就接收/忽略):
                # ① 上行流量保活链路中间盒; ② 推送成功即证 TCP 存活
                # (死连接 send 抛异常 → send_message 内部标死+断开),
                # 刷新 ConnectionInfo.last_heartbeat → 活跃连接不再被误回收。
                for _sid in list(self.active_connections.keys()):
                    ...
                    if _ok:
                        _conn = self.active_connections.get(_sid)
                        if _conn:
                            _conn.update_heartbeat()
