...
处理系统命令（!命令）执行
使用 v2 沙箱统一入口 (SandboxFacade) 执行用户命令
"""
...
class SystemCommandHandler:
    ...
    def __init__(self, send_message_callback):
        ...
        logger.info("[SYSTEM_COMMAND] SandboxFacade 已就绪 (v2 Provider 架构)")
    ...
    async def handle_command(
        ...
        try:
            ...
            ctx = from_user_workspace(user_id, user_workspace.absolute_path)
            result = SandboxFacade.execute_command(command, ctx)
