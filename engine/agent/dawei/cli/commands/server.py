...
@click.option(
    ...
    default=None,
    help="Log file path for daemon mode (default: ~/.normnomos/logs/dawei-server-<port>.log)",
)
@click.option(
    "--pid-file",
    type=click.Path(path_type=Path),
...
def server_start(ctx, host, port, reload, workers, log_level, super, force_kill, daemon, log_file, pid_file, sidecar):
    ...
    if is_port_in_use(port, host):
        ...
        if process_info:
            ...
            # 非交互模式（--sidecar 或 stdin 非 tty，例如被桌面 host 拉起）：
            # 绝不能走到 click.confirm 等 stdin 输入，否则会挂死或抛 Abort。
            # 直接记日志并以非零码退出，交由宿主重试（每次换一个新端口）。
            if sidecar or not sys.stdin.isatty():
                click.echo()
                click.echo(
                    click.style(
                        "❌ Port is in use and cannot prompt in non-interactive mode. Aborting.",
                        fg="red",
                    ),
                    err=True,
                )
    ...
    try:
        ...
        # NOTE: os/sys are imported at module top — do NOT re-import them here.
        # A function-scope `import os`/`import sys` would shadow the module-level
        # binding and make every earlier reference (daemon guard, port-in-use
        # check, non-interactive --sidecar exit) raise UnboundLocalError.
        import signal
        ...
        # Ignore SIGPIPE — prevents BrokenPipeError when the parent process
        # (e.g. Tauri sidecar) closes the stdout/stderr pipe.
        # Python defaults SIGPIPE to SIG_IGN, but PyInstaller/AppImage may reset it.
        # SIGPIPE does not exist on Windows — skip silently.
        if hasattr(signal, "SIGPIPE"):
            signal.signal(signal.SIGPIPE, signal.SIG_IGN)

        # Redirect stdout/stderr to a log file when running as a sidecar
        # (non-daemon, non-tty). This prevents BrokenPipeError if the parent
        # process (Tauri AppImage) closes or can't keep up with the pipe.
        if not daemon and not sys.stdout.isatty():
            _log_dir = os.path.expanduser("~/.normnomos/logs")
            os.makedirs(_log_dir, exist_ok=True)
            _log_path = os.path.join(_log_dir, f"dawei-sidecar-{port}.log")
            _log_file = open(_log_path, "a", encoding="utf-8")  # noqa: SIM115
            sys.stdout = _log_file
            sys.stderr = _log_file
            click.echo(f"[sidecar] stdout/stderr redirected to {_log_path}", err=True)
        ...
        # Configure uvicorn
        # 注意: uvicorn 的 reload 仅在 app 为 import 字符串时生效
        # (Config.should_reload = isinstance(self.app, str) and self.reload)。
        # 直接传 create_app() 的对象会静默禁用 --reload。
        if reload:
            app = "dawei.server_app:create_app"
            factory = True
        else:
            app = create_app()
            factory = False
        ...
        if not daemon:
            ...
            click.echo(click.style(f"http://localhost:{port}/app-ui/", fg="red", bold=True))
            click.echo()
            click.echo("Press Ctrl+C to stop the server")
            click.echo()

        # Run server
        # 注意: reloader 只在 uvicorn.run() 顶层函数里启动；直接调用 server.run()
        # 会绕过文件监听（--reload 静默失效）。这里复刻 uvicorn.run() 的启动分支。
...
@click.option("--port", "-p", type=int, default=8431, help="Server port")
