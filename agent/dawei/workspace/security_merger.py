...
class SecuritySettingsMerger:
    ...
    @staticmethod
    def merge(user_settings: UserSecuritySettings, workspace_settings: dict[str, Any]) -> dict[str, Any]:
        """合并用户和工作区安全配置
        ...
        # === 路径安全合并 ===
        path_security = SecuritySettingsMerger._merge_path_security(user_settings, workspace_security)

        # === 命令执行安全合并 ===
        command_security = SecuritySettingsMerger._merge_command_security(user_settings, workspace_security)

        # === 沙箱配置合并 ===
        sandbox_config = SecuritySettingsMerger._merge_sandbox_config(user_settings, workspace_security)

        # === 模式权限合并 ===
        mode_permissions = SecuritySettingsMerger._merge_mode_permissions(user_settings, workspace_security)

        # === 工具权限合并 ===
        tool_permissions = SecuritySettingsMerger._merge_tool_permissions(user_settings, workspace_security)

        # === 网络安全合并 ===
        network_security = SecuritySettingsMerger._merge_network_security(user_settings, workspace_security)

        # === 资源限制合并 ===
        resource_limits = SecuritySettingsMerger._merge_resource_limits(user_settings, workspace_security)

        # === 高级选项合并 ===
        advanced_options = SecuritySettingsMerger._merge_advanced_options(user_settings, workspace_security)
    ...
    @staticmethod
    def _merge_path_security(user: UserSecuritySettings, workspace: dict[str, Any]) -> dict[str, Any]:
        """合并路径安全配置"""
        ...
        if conflicting:
            logger.warning(f"Conflicting file extensions: {conflicting}. Denied extensions take precedence.")
            allowed_exts -= conflicting

        return {
            "enable_path_traversal_protection": workspace.get("enablePathTraversalProtection", user.enable_path_traversal_protection),
            "allow_absolute_paths": workspace.get("allowAbsolutePaths", user.allow_absolute_paths),
            "allowed_file_extensions": list(allowed_exts),
            "denied_file_extensions": list(denied_exts),
            "max_file_size_mb": min(user.max_file_size_mb, workspace.get("maxFileSizeMb", user.max_file_size_mb)),
        }

    @staticmethod
    def _merge_command_security(user: UserSecuritySettings, workspace: dict[str, Any]) -> dict[str, Any]:
        """合并命令执行安全配置"""
        ...
        if conflicting:
            logger.warning(f"Conflicting commands: {conflicting}. Denied commands take precedence.")
            allowed_cmds = [cmd for cmd in allowed_cmds if cmd not in conflicting]

        return {
            "enable_command_whitelist": workspace.get("enableCommandWhitelist", user.enable_command_whitelist),
            "use_system_command_whitelist": workspace.get("useSystemCommandWhitelist", user.use_system_command_whitelist),
            "allowed_commands": allowed_cmds,
            "denied_commands": denied_cmds,
            "allow_shell_commands": workspace.get("allowShellCommands", user.allow_shell_commands),
            "allow_background_commands": workspace.get("allowBackgroundCommands", user.allow_background_commands),
            "allow_pipe_commands": workspace.get("allowPipeCommands", user.allow_pipe_commands),
            "command_execution_timeout": min(user.command_execution_timeout, workspace.get("commandExecutionTimeout", user.command_execution_timeout)),
        }

    @staticmethod
    def _merge_sandbox_config(user: UserSecuritySettings, workspace: dict[str, Any]) -> dict[str, Any]:
        """合并沙箱配置"""
        # 如果用户强制执行沙箱，工作区不能禁用
        ...
        return {
            "enable_sandbox": workspace.get("enableSandbox", user.enable_sandbox),
            "sandbox_mode": workspace.get("sandboxMode", user.sandbox_mode),
            "allow_sandbox_fallback": workspace.get("allowSandboxFallback", user.allow_sandbox_fallback),
        }

    @staticmethod
    def _merge_mode_permissions(user: UserSecuritySettings, workspace: dict[str, Any]) -> dict[str, Any]:
        """合并模式权限配置"""
        # 合并禁用的模式（用户级+工作区级）
        ...
        return {
            "enable_mode_restrictions": workspace.get("enableModeRestrictions", user.enable_mode_restrictions),
            "plan_mode_allow_writes": workspace.get("planModeAllowWrites", user.plan_mode_allow_writes),
            "restricted_modes": restricted_modes,
        }

    @staticmethod
    def _merge_tool_permissions(user: UserSecuritySettings, workspace: dict[str, Any]) -> dict[str, Any]:
        """合并工具权限配置"""
        ...
        if conflicting:
            logger.warning(f"Conflicting tool groups: {conflicting}. Disabled groups take precedence.")
    ...
    @staticmethod
    def _merge_network_security(user: UserSecuritySettings, workspace: dict[str, Any]) -> dict[str, Any]:
        """合并网络安全配置"""
        ...
        if conflicting:
            logger.warning(f"Conflicting domains: {conflicting}. Denied domains take precedence.")
            allowed_domains = [d for d in allowed_domains if d not in conflicting]

        return {
            "allow_network_access": workspace.get("allowNetworkAccess", user.allow_network_access),
            "allowed_network_domains": allowed_domains,
            "denied_network_domains": denied_domains,
            "max_network_request_size": min(user.max_network_request_size, workspace.get("maxNetworkRequestSize", user.max_network_request_size)),
        }

    @staticmethod
    def _merge_resource_limits(user: UserSecuritySettings, workspace: dict[str, Any]) -> dict[str, Any]:
        """合并资源限制配置"""
        # 资源限制通常不应被工作区放宽
    ...
    @staticmethod
    def _merge_advanced_options(user: UserSecuritySettings, workspace: dict[str, Any]) -> dict[str, Any]:
        """合并高级安全选项"""
        # 高级选项采用"最安全"原则
        return {
            "enable_security_audit_log": user.enable_security_audit_log or workspace.get("enableSecurityAuditLog", False),
            "require_confirmation_for_dangerous": (user.require_confirmation_for_dangerous or workspace.get("requireConfirmationForDangerous", False)),
            "block_executable_files": (user.block_executable_files or workspace.get("blockExecutableFiles", False)),
            "allow_symlinks_outside_workspace": (user.allow_symlinks_outside_workspace and workspace.get("allowSymlinksOutsideWorkspace", False)),  # 只有两个都允许才允许
        }
