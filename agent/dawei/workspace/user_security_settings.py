...
class UserSecuritySettings:
    ...
    base_allowed_extensions: set[str] = field(default_factory=lambda: {".py", ".txt", ".md", ".json", ".yaml", ".yml", ".toml", ".html", ".css", ".js", ".ts", ".vue", ".jsx", ".tsx", ".xml", ".csv", ".sql"})
    # 全局禁止的文件扩展名（工作区不能允许这些）
    base_denied_extensions: set[str] = field(default_factory=lambda: {".exe", ".dll", ".so", ".dylib", ".app", ".deb", ".rpm", ".scr", ".vbs", ".jar", ".com", ".pif", ".bat", ".sh"})
    ...
    base_enabled_tool_groups: list[str] = field(default_factory=lambda: ["read", "browser", "task_graph", "workflow"])
    # 全局禁用的工具组（工作区不能启用）
    ...
    def from_dict(cls, config_dict: dict[str, Any]) -> "UserSecuritySettings":
        ...
        return cls(
            # 路径安全
            enable_path_traversal_protection=config_dict.get("enablePathTraversalProtection", True),
            allow_absolute_paths=config_dict.get("allowAbsolutePaths", False),
            base_allowed_extensions=set(config_dict.get("baseAllowedExtensions", [".py", ".txt", ".md", ".json", ".yaml", ".yml"])),
            base_denied_extensions=set(config_dict.get("baseDeniedExtensions", [".exe", ".dll", ".so", ".dylib"])),
            ...
            sandbox_mode=config_dict.get("sandboxMode", "docker"),
            ...
            # 模式权限
            enable_mode_restrictions=config_dict.get("enableModeRestrictions", True),
            plan_mode_allow_writes=config_dict.get("planModeAllowWrites", False),
            globally_disabled_modes=config_dict.get("globallyDisabledModes", []),
            # 工具权限
            base_enabled_tool_groups=config_dict.get("baseEnabledToolGroups", ["read", "browser", "task_graph", "workflow"]),
            base_disabled_tool_groups=config_dict.get("baseDisabledToolGroups", []),
            base_disabled_tools=config_dict.get("baseDisabledTools", []),
            # 网络安全
            allow_network_access=config_dict.get("allowNetworkAccess", True),
            base_allowed_domains=config_dict.get("baseAllowedDomains", []),
            base_denied_domains=config_dict.get("baseDeniedDomains", []),
            max_network_request_size=config_dict.get("maxNetworkRequestSize", 10),
            # 资源限制
            max_concurrent_operations=config_dict.get("maxConcurrentOperations", 5),
            max_memory_mb=config_dict.get("maxMemoryMb", 2048),
            max_execution_time=config_dict.get("maxExecutionTime", 300),
            # 高级选项
            enable_security_audit_log=config_dict.get("enableSecurityAuditLog", True),
            require_confirmation_for_dangerous=config_dict.get("requireConfirmationForDangerous", True),
            block_executable_files=config_dict.get("blockExecutableFiles", True),
            allow_symlinks_outside_workspace=config_dict.get("allowSymlinksOutsideWorkspace", False),
            # 工作区覆盖控制
            allow_workspace_override_path_security=config_dict.get("allowWorkspaceOverridePathSecurity", True),
            allow_workspace_override_command_security=config_dict.get("allowWorkspaceOverrideCommandSecurity", True),
            allow_workspace_override_sandbox=config_dict.get("allowWorkspaceOverrideSandbox", True),
            allow_workspace_override_tool_permissions=config_dict.get("allowWorkspaceOverrideToolPermissions", True),
            allow_workspace_override_network_security=config_dict.get("allowWorkspaceOverrideNetworkSecurity", True),
            allow_workspace_override_resource_limits=config_dict.get("allowWorkspaceOverrideResourceLimits", False),
        )
